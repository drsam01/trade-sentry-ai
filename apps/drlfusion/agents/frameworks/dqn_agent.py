# File: apps/drlfusion/agents/frameworks/dqn_agent.py
...
class DQNAgent(BaseAgent):
    ...
    def choose_action(
        ...
        """
        state_tensor = torch.from_numpy(state).float().unsqueeze(0).to(device) # type: ignore
    ...
    def save(self, path: str) -> None:
        ...
        """
        torch.save(self.q_net.state_dict(), path) # type: ignore
    ...
    def load(self, path: str) -> None:
        ...
        """
        self.q_net.load_state_dict(torch.load(path, map_location=device)) # type: ignore
