...
class PPOManager(BaseManager):
    ...
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(self.agent.model.parameters(), lr=self.agent.learning_rate) # type: ignore
    ...
    def update(self, batch: dict) -> dict:
        ...
        states = torch.tensor(np.array(batch["states"]), dtype=torch.float32).to(device) # type: ignore
        actions = torch.tensor(batch["actions"], dtype=torch.long).to(device) # type: ignore
        old_log_probs = torch.tensor(batch["log_probs"], dtype=torch.float32).to(device) # type: ignore
        returns = torch.tensor(batch["returns"], dtype=torch.float32).to(device) # type: ignore
        advantages = torch.tensor(batch["advantages"], dtype=torch.float32).to(device) # type: ignore
        ...
        ratios = torch.exp(new_log_probs - old_log_probs) # type: ignore
        surrogate1 = ratios * advantages
        surrogate2 = torch.clamp(ratios, 1 - self.clip_epsilon, 1 + self.clip_epsilon) * advantages # type: ignore
        policy_loss = -torch.min(surrogate1, surrogate2).mean() # type: ignore
        ...
        total_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.agent.model.parameters(), max_norm=0.5) # type: ignore
        self.optimizer.step()
        self.scheduler.step()
    ...
    def load(self, path: str) -> None:
        ...
        checkpoint = torch.load(path, map_location=torch.device("cpu")) # type: ignore
