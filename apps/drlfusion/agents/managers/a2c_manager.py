# File: apps/drlfusion/agents/frameworks/a2c_manager.py
...
class A2CManager(BaseManager):
    ...
    def update(self, batch: Dict[str, List]) -> Dict[str, float]:
        ...
        """
        states = torch.tensor(batch["states"], dtype=torch.float32).to(self.device) # type: ignore
        actions = torch.tensor(batch["actions"], dtype=torch.long).to(self.device) # type: ignore
        returns = torch.tensor(batch["returns"], dtype=torch.float32).to(self.device) # type: ignore

        self.agent.optimizer.zero_grad()
        probs, values = self.agent.model(states)
        dist = torch.distributions.Categorical(probs) # type: ignore
