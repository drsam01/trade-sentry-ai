...
from apps.drlfusion.agents.frameworks.sac_agent import SACAgent, Critic
...
#        tau: float = 0.005
        ...
        self.actor_opt = torch.optim.Adam(self.agent.actor.parameters(), lr=training_config.get("ACTOR_LR", 1e-4)) # type: ignore
        self.critic_1_opt = torch.optim.Adam(self.critic_1.parameters(), lr=training_config.get("CRITIC_LR", 1e-4)) # type: ignore
        self.critic_2_opt = torch.optim.Adam(self.critic_2.parameters(), lr=training_config.get("CRITIC_LR", 1e-4)) # type: ignore
    ...
    def update(self, batch: Dict[str, list]) -> Dict[str, float]:
        states = torch.tensor(np.array(batch["states"]), dtype=torch.float32).to(self.device) # type: ignore
        actions = torch.tensor(np.array(batch["actions"]), dtype=torch.float32).to(self.device) # type: ignore
        # Ensure shape is [batch_size, action_dim]
        if actions.ndim == 1:
            actions = actions.unsqueeze(1)
        rewards = torch.tensor(batch["rewards"], dtype=torch.float32).unsqueeze(1).to(self.device) # type: ignore
        next_states = torch.tensor(np.array(batch["next_states"]), dtype=torch.float32).to(self.device) # type: ignore
        dones = torch.tensor(batch["dones"], dtype=torch.float32).unsqueeze(1).to(self.device) # type: ignore

        # Critic loss
        with torch.no_grad(): # type: ignore
            mean, log_std = self.agent.actor(next_states)
            std = log_std.exp()
            dist = torch.distributions.Normal(mean, std) # type: ignore
            next_actions = torch.tanh(dist.rsample()) # type: ignore
            log_probs = dist.log_prob(next_actions).sum(-1, keepdim=True)

            target_q1 = self.target_critic_1(next_states, next_actions)
            target_q2 = self.target_critic_2(next_states, next_actions)
            target_min = torch.min(target_q1, target_q2) - self.alpha * log_probs # type: ignore
        ...
        std = log_std.exp()
        dist = torch.distributions.Normal(mean, std) # type: ignore
        actions_sampled = torch.tanh(dist.rsample()) # type: ignore
        ...
        q_pi = torch.min(q1_pi, q2_pi) # type: ignore
