...
from core.utils.stop_criteria import TrainingStopCriteria
from core.utils.rollout_tools import safe_step, safe_reset
...
class BaseTrainer(ABC):
    ...
    def collect_experiences(self) -> Dict[str, list]:
        ...
        with torch.no_grad(): # type: ignore
            state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0).to(self.manager.agent.device) # type: ignore
    ...
    def log_and_maybe_stop(
        ...
        return self.stopper.check(epoch, train_reward, val_reward) # type: ignore
